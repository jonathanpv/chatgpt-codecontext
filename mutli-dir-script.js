...
(function() {
    ...
    function injectCustomUI() {
        ...
        customUIContainer.style.width = '350px'; // Increased width to accommodate directories
    ...
    // Function to display the file list as checkboxes, grouped by directory
    function displayFileList(filesByDirectory) {
        const fileListContainer = document.getElementById('file-list');
        if (!fileListContainer) return;
        ...
        directories.forEach(directory => {
            const dirHeader = document.createElement('h4');
            dirHeader.innerText = directory;
            dirHeader.style.marginTop = '10px';
            dirHeader.style.marginBottom = '5px';
            dirHeader.style.fontFamily = 'inherit';
            dirHeader.style.color = 'inherit';
            fileListContainer.appendChild(dirHeader);

            const files = filesByDirectory[directory];
            if (files.length === 0) {
                const noFiles = document.createElement('p');
                noFiles.innerText = 'No files in this directory.';
                noFiles.style.marginLeft = '10px';
            ...
            files.forEach(file => {
                ...
                checkbox.value = file;
                checkbox.dataset.directory = directory; // Store directory info
                ...
                checkbox.addEventListener('change', function() {
                    if (this.checked) {
                        ...
                        fetchFileContent(this.value, this.dataset.directory);
                    } else {
                        this.style.transform = 'scale(1.2)';
                        setTimeout(() => {
                            this.style.transform = 'scale(1)';
                        }, 200);
                        removeFileContent(this.value, this.dataset.directory);
                    }
                });
    ...
    function fetchFileContent(filename, directory) {
        GM_xmlhttpRequest({
            method: "GET",
            url: `${SERVER_URL}/get_file_content?filename=${encodeURIComponent(filename)}&directory=${encodeURIComponent(directory)}`,
            onload: function(response) {
                if (response.status === 200) {
                    const data = JSON.parse(response.responseText);
                    appendToPrompt(data.content, filename, directory);
                } else {
    ...
    // Function to remove file content from the prompt
    function removeFileContent(filename, directory) {
        const promptContainer = document.querySelector('#prompt-textarea');
        if (promptContainer) {
            // Use a unique identifier for each file based on directory and filename
            const contentRegex = new RegExp(`<p>=== ${directory}/${filename} ===</p>[\\s\\S]*?(?=<p>===|$)`, 'g');
    ...
    // Function to append file content to the prompt
    function appendToPrompt(content, filename, directory) {
        const promptContainer = document.querySelector('#prompt-textarea');
        if (promptContainer) {
            // Create a unique identifier for the file content
            const contentToAdd = `<p>=== ${directory}/${filename} ===</p><p>${content.replace(/\n/g, '</p><p>')}</p>`;
    ...
    function refreshPrompt() {
        ...
        const existingContents = promptContainer.querySelectorAll('p');
        existingContents.forEach(p => {
            if (p.textContent.startsWith('=== ')) {
                p.nextElementSibling?.remove(); // Remove the content paragraph
                p.remove(); // Remove the header
            }
        });

        // Re-fetch and append content for all selected files
        selectedCheckboxes.forEach(checkbox => {
            fetchFileContent(checkbox.value, checkbox.dataset.directory);
        });
    }
